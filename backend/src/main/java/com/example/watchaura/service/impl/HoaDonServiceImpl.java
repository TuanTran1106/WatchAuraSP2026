...
public class HoaDonServiceImpl implements HoaDonService {
    ...
    public HoaDonDTO updateTrangThaiDonHang(Integer id, String trangThaiDonHang) {
        ...
        // DA_THANH_TOAN_ONL: tiền đã về online, trừ kho ngay và khóa trạng thái

        // Không cho chuyển sang trạng thái khác (khóa luôn như DA_HUY, DA_THANH_TOAN)

        if ("DA_THANH_TOAN_ONL".equals(effectiveCurrent) && !"DA_THANH_TOAN_ONL".equals(newStatus)) {

            throw new RuntimeException("Đơn đã thanh toán online thành công, không thể đổi trạng thái.");
        ...
        // Hoàn tồn kho khi hủy đơn (nếu đơn đã xác nhận/thanhtoán); KHÔNG cho hủy khi đã thanh toán tại quầy
        ...
        if ("DA_HUY".equals(newStatus) && !"DA_HUY".equals(effectiveCurrent)) {
            ...
            // DA_THANH_TOAN_ONL: cho phép hủy (tiền cần hoàn về VNPay)

            if ("DA_THANH_TOAN_ONL".equals(effectiveCurrent)) {

                // Không cần hoàn kho vì chưa trừ kho ở trạng thái này

            } else if ("DA_XAC_NHAN".equals(effectiveCurrent)) {
